...
GetProcessUserDirectoryTableBase(PEPROCESS Process, CR3* UserDirectoryTableBase)
{
    ...
    else if (Version.dwMajorVersion == 12 && Version.dwMinorVersion == 0 && Version.dwBuildNumber == 19045) Offset = 0x388;
    else return STATUS_NOT_SUPPORTED;
...
NTSTATUS
GetProcessAddressPolicy(PEPROCESS Process, BOOLEAN* AddressPolicy)
{
    RTL_OSVERSIONINFOW Version = { .dwOSVersionInfoSize = sizeof(RTL_OSVERSIONINFOW) };
    RtlGetVersion(&Version);

    UINT64 Offset;
         if (Version.dwMajorVersion == 10 && Version.dwMinorVersion == 0 && Version.dwBuildNumber == 19044) Offset = 0x390;
    else if (Version.dwMajorVersion == 10 && Version.dwMinorVersion == 0 && Version.dwBuildNumber == 19045) Offset = 0x390;
    else return STATUS_NOT_SUPPORTED;

    *AddressPolicy = *(BOOLEAN*)((UINT64)Process + Offset);
    return STATUS_SUCCESS;
}
